...
Producto.update = function (Producto, resultado) {
    sql.query(`UPDATE Productos SET Nombre = ??, Descripcion = ??, Precio = ??, Tipo = ?? WHERE id = ??`, [Producto.nombre], [Producto.descripcion], [Producto.precio], [Producto.tipo], [Producto.id], (err, sqlRes) => {
        if (err) {
            console.log(err);
            resultado(err, null);
        }
